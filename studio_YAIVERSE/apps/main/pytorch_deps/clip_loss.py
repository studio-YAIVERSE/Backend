...
class CLIPLoss(torch.nn.Module):
    ...
    def compute_img2img_direction(self, source_images: torch.Tensor, target_images: list) -> torch.Tensor:
        with torch.no_grad():
            # js
            src_encoding = self.get_image_features(source_images)  # input image embedding -> E(I)
            src_encoding = src_encoding.mean(dim=0, keepdim=True)  # E(I)/N_r
    ...
    def set_text_features(self, source_class: str, target_class: str) -> None:
        ...
        target_features = self.get_text_features(target_class).mean(dim=0, keepdim=True)
    ...
    def clip_directional_loss(self, src_img: torch.Tensor, source_class: str, target_img: torch.Tensor,
        ...
        # print('DEBUG : ', src_encoding.shape, target_encoding.shape)
        ...
        # print('DEBUG : ', edit_direction.shape)
